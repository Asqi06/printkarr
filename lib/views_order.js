...
function documentPreview(url, pages, range) {
  ...
  }).join(', ') : `all ${esc(pages)} pages`;
  const selection = range ? `pages ${selected}${pages ? ` of ${esc(pages)}` : ''}` : selected;
  return `<details class="document-preview">
    <summary><span>Preview your PDF</span><small>Check the actual pages before printing</small></summary>
    <div class="document-preview-body">
      ...
      <a href="${esc(url)}" target="_blank" rel="noopener">Open PDF in a new tab ↗</a>
    </div>
  </details>`;
}
...
export function optionsStep(user, draft, addresses) {
  ...
  return layout({
    title: 'Print options', user, extraCss: '/customer.css', extraHead: DELIVERY_MAP_HEAD, active: '/customer/orders',
    body: `
...
export function summaryStep(user, draft, s, q, packCover = null) {
  return layout({
    ...
    <div class="card rv" style="margin-top:18px">
      ...
      ${documentPreview(`/customer/orders/new/${encodeURIComponent(draft.id)}/preview.pdf`, draft.pages, s.range)}
      ...
      <div class="sumrow"><span>${s.zone === 'pickup' ? 'Kiosk collection' : `Delivery · ${esc(s.zoneLabel)} · approx. ${q.deliveryKm} km from base`}</span><span>${q.deliveryFee === 0 ? 'FREE' : rs(q.deliveryFee)}</span></div>
      ...
      ${packCover ? `<p class="muted mono" style="font-size:10px;margin-top:8px">Pack covers printing — delivery only. Coupons skip when a pack applies.</p>` : ''}
...
export function payStep(user, order, opts = {}) {
  ...
  const printSubtotal = Math.max(0, Number(order.subtotal ?? (Number(order.total || 0) - Number(order.deliveryFee || 0) - Number(order.lateNightFee || 0) - Number(order.surgeFee || 0) + Number(order.couponDiscount || 0))) + Number(order.discount || 0));
  ...
  return layout({
    ...
    <div class="receipt-printer" aria-label="Printed order summary">
      ...
      <article class="receipt-paper">
        ...
        ${documentPreview(`/customer/orders/${encodeURIComponent(order.id)}/preview.pdf`, order.filePages, order.pageRange)}
