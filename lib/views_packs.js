...
export function packsPage(user, { subs, walletBalance, livePay, refErr, refOff = 20 }) {
  ...
  const errBox = refErr ? `<p class="login-err rv" role="alert" style="margin-top:12px">${esc(refErr)}</p>` : '';
  const cards = PACKS.map((p) => {
    const save = Math.max(0, p.market - p.price);
    return `<article class="card rv">
      <p class="eyebrow">${p.id === 'M' ? 'MOST POPULAR · ' : ''}${esc(p.blurb)}</p>
      <h3 class="display" style="font-size:1.6rem">${esc(p.name.replace('Semester Pack ', ''))}</h3>
      <p class="display" style="font-size:2.5rem;margin:8px 0">${rs(p.price)}</p>
      <div class="sumrow"><span>B&amp;W pages</span><b>${p.bw}</b></div>
      <div class="sumrow"><span>Colour pages</span><b>${p.color}</b></div>
      <div class="sumrow"><span>File claims</span><b>${p.files}</b></div>
      <div class="sumrow"><span>Pay today to start</span><b>${rs(BOOKING_FEE)}</b></div>
      <div class="sumrow"><span>Then pay</span><b>${rs(p.price - BOOKING_FEE)}</b></div>
      <div class="sumrow disc"><span>Compared with ${rs(p.market)} estimate</span><span>Save ${rs(save)}</span></div>
      <form method="POST" action="/customer/packs/${p.id}/subscribe" style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap;align-items:end">
        ...
        <div class="field" style="margin:0"><label>Referral (first pack, up to −${rs(refOff)})</label>
        <input name="referral" placeholder="K7Q2M4" maxlength="12" style="text-transform:uppercase;max-width:130px"></div>
  ...
  return layout({
    ...
    body: `
    <p class="eyebrow rv">Semester print packs</p>
    <h1 class="display rv" style="font-size:clamp(2rem,6vw,3.6rem)">You’re going to<br><em>print anyway.</em></h1>
