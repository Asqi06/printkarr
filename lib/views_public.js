...
export function landing({ pagesWeek, pricing, queueDepth }) {
  ...
  return shell({
    ...
    extra: `<style>
      ...
      .landing-top { position: sticky; top: 0; z-index: 100; display: flex; justify-content: space-between; align-items: center; padding: 12px var(--pad); background: rgba(250,245,234,.92); backdrop-filter: blur(10px); border-bottom: 2px solid var(--line); }
      .hero-min { max-width: 1080px; margin: 0 auto; padding: clamp(28px,6vh,56px) var(--pad) 18px; display: grid; grid-template-columns: 1.15fr .85fr; gap: clamp(20px,4vw,44px); align-items: center; }
      .hero-kicker { font-family: var(--font-m); font-size: 11px; letter-spacing: .2em; text-transform: uppercase; color: var(--muted); display: flex; gap: 10px; align-items: center; }
      .hero-kicker i { width: 9px; height: 9px; border-radius: 50%; background: var(--leaf); animation: throb 1.6s infinite; }
      .hero-min .display { font-size: clamp(3rem,7vw,5.6rem); line-height: .9; }
      .hero-min .display em { color: var(--tang); }
      .hero-sub { max-width: 42ch; color: var(--muted); margin-top: 12px; font-size: 15px; line-height: 1.5; }
      .hero-cta { display: inline-flex; margin-top: 18px; font-size: 15px; padding: 18px 28px; box-shadow: 6px 6px 0 var(--ink); }
      .hero-cta:hover { transform: translate(2px,2px); box-shadow: 3px 3px 0 var(--ink); }
      .hero-help { font-family: var(--font-m); font-size: 11px; color: var(--muted); margin-top: 10px; }
      .hero-trust { display: flex; gap: 8px; flex-wrap: wrap; margin-top: 16px; }
      .hero-trust span { font-family: var(--font-m); font-size: 10px; font-weight: 700; letter-spacing: .08em; text-transform: uppercase; border: 2px solid var(--line); border-radius: 999px; padding: 7px 12px; background: var(--paper); }
      .hero-trust b { color: var(--ink); }
      .hero-art { position: relative; aspect-ratio: 1.05; border: 3px solid var(--ink); border-radius: 18px; background: var(--paper-2); overflow: hidden; transform: rotate(-.6deg); }
      .hero-art .sheet { position: absolute; left: 50%; top: 50%; width: 62%; aspect-ratio: .72; background: var(--paper); border: 2px solid var(--ink); border-radius: 10px; box-shadow: 6px 6px 0 rgba(0,0,0,.08); }
      .hero-art .sheet.s1 { transform: translate(-50%,-52%) rotate(-6deg); }
      .hero-art .sheet.s2 { transform: translate(-48%,-48%) rotate(4deg); background: #fff; }
      .hero-art .sheet.s3 { transform: translate(-50%,-44%) rotate(-1deg); display: flex; align-items: center; justify-content: center; font-family: var(--font-m); font-weight: 700; font-size: 12px; }
      .sticker { position: absolute; font-family: var(--font-m); font-weight: 700; font-size: 11px; letter-spacing: .06em; text-transform: uppercase; border: 2px solid var(--ink); border-radius: 999px; padding: 7px 10px; transform: rotate(-3deg); }
      .sticker.y { background: var(--sun); left: 10px; top: 14px; animation: wiggle 2.8s ease-in-out infinite; }
      .sticker.o { background: var(--tang); color: var(--paper); right: 12px; bottom: 14px; animation: wiggle 3.2s ease-in-out infinite .6s; }
      @keyframes wiggle { 0%,100% { transform: rotate(-3deg) translateY(0)} 50% { transform: rotate(2deg) translateY(-4px)} }
      .steps-min { max-width: 1080px; margin: 14px auto 0; padding: 0 var(--pad); display: grid; grid-template-columns: repeat(3,1fr); gap: 12px; }
      .steps-min .step { border: 2px solid var(--ink); border-radius: 16px; padding: 16px 14px; background: var(--paper); position: relative; transition: transform .2s var(--ez-out); }
      .steps-min .step:nth-child(2){ transform: rotate(.4deg); background: #fff; }
      .steps-min .step:nth-child(3){ transform: rotate(-.3deg); }
      .steps-min .step:hover { transform: translateY(-3px) rotate(0deg); }
      .steps-min .n { font-family: var(--font-m); font-size: 11px; letter-spacing: .16em; color: var(--tang); font-weight: 700; }
      .steps-min h3 { font-weight: 900; text-transform: uppercase; letter-spacing: -.01em; margin-top: 6px; font-size: 1rem; }
      .steps-min p { color: var(--muted); font-size: 13px; margin-top: 4px; }
      .bonus-min { display: flex; justify-content: space-between; align-items: center; gap: 12px; max-width: 1080px; margin: 14px auto 0; padding: 12px 14px; border: 2px dashed var(--ink); border-radius: 12px; font-family: var(--font-m); font-size: 11.5px; font-weight: 700; letter-spacing: .05em; text-transform: uppercase; text-decoration: none; background: var(--sun); color: var(--ink); }
      .contact-min { max-width: 1080px; margin: 14px auto 0; padding: 14px; border: 2px solid var(--ink); border-radius: 14px; display: flex; flex-wrap: wrap; gap: 10px 20px; align-items: center; justify-content: space-between; background: var(--paper); }
      .contact-min .num { font-weight: 900; font-size: 1.35rem; letter-spacing: -.02em; text-decoration: none; color: var(--ink); }
      ...
      @media (max-width: 860px){ .hero-min{ grid-template-columns: 1fr; } .hero-art{ order: -1; max-width: 520px; margin: 0 auto; } .steps-min{ grid-template-columns: 1fr; } }
    </style>`,
    body: `
    <div class="landing">
