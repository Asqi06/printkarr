...
${active === '/' || active === '/order' ? '' : '<div class="sticky-cta"><a href="/order">Upload PDF &amp; Print →</a></div>'}
${pkPublicFooter()}
</div>
...
export function landing({ pricing, maxMb }) {
  ...
  return shell({ active: '/', title: 'Need it on paper?', desc: 'Upload a PDF and print from your phone. Kiosk pickup or local delivery around Vapi. See the full price before payment.', body: `
    <div class="marketing-wrap conversion-home">
      <section class="studio-hero" aria-labelledby="home-title">
        <div class="hero-copy">
          ...
          <h1 id="home-title">Need it<br><em>on paper?</em></h1>
          <p class="hero-description">Upload your PDF. Choose what prints. Pick it up at a kiosk or get it delivered.</p>
        ...
        <form class="hero-upload rv" method="POST" action="/order/upload" enctype="multipart/form-data">
          ...
          <label class="drop" for="hero-doc"><b>Choose a PDF</b><span class="muted">Up to ${mb} MB · price shown after upload</span></label>
      ...
      <section class="kiosk-feature" id="kiosks"><div><p class="eyebrow">04 / THE KIOSK</p><h2>The Xerox shop.<br><em>Without the shop.</em></h2><p>Scan the QR. Upload your PDF. Pay on your phone. Collect your print at the kiosk.</p><p class="kiosk-steps">SCAN <span>→</span> UPLOAD <span>→</span> PAY <span>→</span> COLLECT</p><a class="btn loud big" href="/order">Start a kiosk print →</a><small>No app required · kiosk pickup is free</small></div><img src="/images/kiosk-hero.png" alt="PrintKarr self-service printing kiosk" loading="lazy"></section>
      ...
      <section class="home-section" id="faq"><div class="home-section-head"><p class="eyebrow">GOOD TO KNOW</p><h2>Before you<br><em>upload.</em></h2></div><div class="faq">
        ...
        <details><summary>How do I collect my print?</summary><p>Choose kiosk pickup for ₹0. Track the order on your phone and scan the pickup QR at the kiosk when it is ready.</p></details>
      </div></section>
...
export function orderPage({ draft, pricing, error, maxMb, surcharges = {} }) {
  ...
  return shell({
    ...
    <section class="pub-wrap">
      ...
      ${d ? `
      <div class="card rv filechip" style="background:var(--pale)"><b>${esc(d.document)}</b><span class="muted" style="font-size:12px">${d.pages} pages detected · <a class="rowlink" href="/order">Choose a different file</a></span></div>
      <form class="quick-order" method="POST" action="/order/options">
        ...
        <div class="card rv quick-print">
          <p class="eyebrow">YOUR PDF</p><p class="quick-print-default">${d.pages} pages detected · A4</p>
          ...
          <details class="order-more"><summary>Change settings or delivery area</summary><div class="order-more-body">
            ...
            <div class="field"><label for="range">Pages to print</label><input id="range" name="range" autocomplete="off" placeholder="All ${d.pages} pages · or e.g. 1, 3-5" aria-describedby="range-hint"><p class="field-hint" id="range-hint">Leave empty for all pages. Enter one page or ranges separated by commas.<span class="err" id="range-err"></span></p></div>
