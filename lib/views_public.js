  ...
  return shell({
    ...
    <div style="max-width:72rem;margin:0 auto;padding:110px 16px 0">
      <section class="pk-hero rv"><div class="grid-veil" style="position:absolute;inset:0;opacity:.7"></div>
        ...
        <div class="pk-kiosk pk-kiosk3d" id="kiosk3d" data-fbx="/models/kiosk.fbx?v=${ASSET_V}"><img class="kiosk-img" data-kiosk-fallback src="/images/kiosk-hero.png" alt="PrintKarr self-service printing kiosk" loading="eager" onerror="this.remove()"></div>
      </section>
      ...
      (function () {
        ...
        Promise.all([import('three'), import('three/addons/loaders/FBXLoader.js'), import('three/addons/environments/RoomEnvironment.js')]).then(function (mods) {
          var THREE = mods[0];
          ...
          try {
            ...
            scene.environment = pmrem.fromScene(new mods[2].RoomEnvironment(), 0.04).texture;
          } catch (e2) {}
          scene.add(new THREE.HemisphereLight(0xffffff, 0xdfe7f5, 0.85));
          scene.add(new THREE.AmbientLight(0xffffff, 0.28));
          var key = new THREE.DirectionalLight(0xffffff, 2.4); key.position.set(4, 7, 6); scene.add(key);
          var fill = new THREE.DirectionalLight(0xd6e4ff, 0.9); fill.position.set(-5, 2.5, 4); scene.add(fill);
          var front = new THREE.DirectionalLight(0xffffff, 0.55); front.position.set(0, 1.5, 8); scene.add(front);
          var rim = new THREE.DirectionalLight(0xffffff, 1.2); rim.position.set(-1, 4, -6); scene.add(rim);
          ...
          function render() {
            ...
            rig.scale.setScalar(0.96 + Math.sin(es * Math.PI) * 0.08);
            rig.position.y = baseY - e * 0.3;
            shadow.material.opacity = 0.55 - e * 0.2;
          ...
          new FBXLoader().load(stage.getAttribute('data-fbx'), function (obj) {
            ...
            inner.scale.setScalar(3.6 / Math.max(size.x, size.y, size.z));
            rig.add(inner);
            // Reduce shininess/specular so the model looks matte/not-glassy
            inner.traverse(function (child) {
              if (child.isMesh) {
                var mat = child.material;
                if (mat) {
                  if (Array.isArray(mat)) mat = mat[0];
                  mat.shininess = 0; // remove specular highlight
