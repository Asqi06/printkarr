...
function shell({ title, body, desc, active }) {
  const GLO_W = 1271, GLO_H = 599;
  function bellHeights(n, peak, valley) {
    const out = [];
    const mid = (n - 1) / 2;
    for (let i = 0; i < n; i++) {
      const t = mid === 0 ? 0 : Math.abs(i - mid) / mid;
      out.push(peak * GLO_H * (valley + (1 - valley) * (1 - Math.pow(t, 1.24))));
    }
    return out;
  }
  const GLO_STOPS = [[0, '#340B05'], [0.1827, '#0358F7'], [0.2837, '#5092C7'], [0.4135, '#E1ECFE'], [0.5866, '#FFD400'], [0.6827, '#FA3D1D'], [0.8029, '#FD02F5'], [1, '#FFC0FD00']];
  const GLO_BARS = 9;
  const glowBand = `<div class="pk-glow" aria-hidden="true"><svg viewBox="0 0 ${GLO_W} ${GLO_H}" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="pk-grad" x1="0" y1="1" x2="0" y2="0">${GLO_STOPS.map((s) => `<stop offset="${s[0]}" stop-color="${s[1]}"/>`).join('')}</linearGradient><filter id="pk-blur" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="15"/></filter></defs>${bellHeights(GLO_BARS, 0.98, 0.55).map((barH, i) => `<g filter="url(#pk-blur)"><rect x="${(i * GLO_W / GLO_BARS).toFixed(1)}" y="${(GLO_H - barH).toFixed(1)}" width="${(GLO_W / GLO_BARS * 1.23).toFixed(1)}" height="${barH.toFixed(1)}" fill="url(#pk-grad)"/></g>`).join('')}</svg></div>`;
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<footer class="pk-footer" style="padding-bottom:40vh"><div class="pk-footer-grid">
...
</div><div class="pk-footer-bottom"><p>© ${new Date().getFullYear()} PrintKarr · PrintKarr Technologies Private Limited</p><div style="display:flex;gap:16px"><a href="/terms">Terms &amp; Conditions</a><a href="/privacy">Privacy Policy</a></div></div>${glowBand}</footer>
</div>
  ...
  return shell({
    ...
    <div style="max-width:72rem;margin:0 auto;padding:110px 16px 0">
      <section class="pk-hero rv"><div class="grid-veil" style="position:absolute;inset:0;opacity:.7"></div>
        ...
        <div class="pk-kiosk"><img class="kiosk-img" src="/images/kiosk-hero.png" alt="PrintKarr self-service printing kiosk" loading="eager"></div>
      </section>
      <div class="ribbon" role="region" aria-label="PrintKarr highlights: instant prints, low prices, always open"><div class="ribbon-track" aria-hidden="true">${('<span>' + RIBBON_TX + '</span>').repeat(12)}</div></div>
