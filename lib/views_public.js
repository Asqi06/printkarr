...
function stepsHtml() {
  ...
  return `<div class="pk-steps-wrap"><div class="step-line" aria-hidden="true"></div><div class="pk-steps">${steps.map((s, i) => `<article class="pk-step rv" data-tilt style="--d:${(0.05 + i * 0.1).toFixed(2)}s"><span class="n">${s.n}</span><h3>${s.t}</h3><p>${s.d}</p><span class="tag">${s.tag}</span></article>`).join('')}</div></div>`;
}
...
export function landing({ pagesWeek, pricing, queueDepth }) {
  ...
  return shell({
    ...
    <div style="max-width:72rem;margin:0 auto;padding:110px 16px 0">
      <section class="pk-hero rv"><svg class="agp" aria-hidden="true"><defs><pattern id="agp-grid" width="40" height="40" patternUnits="userSpaceOnUse" x="-1" y="-1"><path d="M.5 40V.5H40" fill="none" stroke-dasharray="0"/></pattern></defs><rect width="100%" height="100%" fill="url(#agp-grid)"/><g class="agp-cells"></g></svg>
      ...
      (function () {
        ...
        var agp = document.querySelector('.pk-hero .agp');
        if (agp && !reduce) {
          var NS = 'http://www.w3.org/2000/svg';
          var AW = 40, AH = 40, AN = 30, AD = 3, ARD = 1, AMAX = 0.1;
          var cellsG = agp.querySelector('.agp-cells');
          function agpPlace(r) {
            var w = agp.clientWidth || 1, h = agp.clientHeight || 1;
            var cols = Math.max(Math.floor(w / AW), 1), rows = Math.max(Math.floor(h / AH), 1);
            r.setAttribute('x', Math.floor(Math.random() * cols) * AW + 1);
            r.setAttribute('y', Math.floor(Math.random() * rows) * AH + 1);
          }
          function agpBlink(r, i) {
            agpPlace(r);
            if (typeof r.animate !== 'function') return;
            var total = AD * 2 + ARD;
            var a = r.animate(
              [{ opacity: 0 }, { opacity: AMAX, offset: AD / total }, { opacity: 0, offset: (AD * 2) / total }, { opacity: 0 }],
              { duration: total * 1000, delay: i * 100, easing: 'ease-in-out' }
            );
            a.onfinish = function () { agpBlink(r, i); };
          }
          if (cellsG && 'animate' in cellsG) {
            for (var gi = 0; gi < AN; gi++) {
              var cell = document.createElementNS(NS, 'rect');
              cell.setAttribute('width', AW - 1);
              cell.setAttribute('height', AH - 1);
              cell.setAttribute('class', 'agp-cell');
              cellsG.appendChild(cell);
              agpBlink(cell, gi);
            }
          }
        }
