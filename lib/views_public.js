...
function stepsHtml() {
  ...
  return `<div class="pk-steps">${steps.map((s, i) => `<article class="pk-step rv" style="--d:${(i * 0.07).toFixed(2)}s"><span class="n">${s.n}</span><h3>${s.t}</h3><p>${s.d}</p><span class="tag">${s.tag}</span></article>`).join('')}</div>`;
}
