...
export function printedAt(order) {
  if (order.status === 'CANCELLED') return null;
  const event = Array.isArray(order.history) && order.history.find((entry) => entry.to === 'PRINTED');
  return event?.at || (order.status === 'PRINTED' ? order.updatedAt || null : null);
}
