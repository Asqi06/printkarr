...
export function customerDashboard(user, { current, pricing, notes, lastDoc, packsHtml }) {
  ...
  const card = current
    ...
    : `<a class="card sun rv" href="/customer/orders/new" style="text-decoration:none">
        <div class="stat"><div class="v">Ready to print?</div><div class="k">Start your first print order →</div></div>
      </a>`;
  ...
  return layout({
    ...
    <div class="chips rv">
      ...
      <a class="chip" href="/customer/packs">Semester packs</a>
      <a class="chip" href="/customer/referrals">Refer &amp; earn ₹20 cash</a>
...
export function ordersList(user, { tab, counts, orders }) {
  ...
  const cards = orders.map((o) => `
    <article class="card rv">
      ...
      <div style="display:flex;gap:8px;align-items:center;margin-top:6px"><a class="rowlink" href="/customer/orders/${o.id}">View details →</a>${['CREATED','PAYMENT_PENDING'].includes(o.status) ? `<a class="btn sun" style="padding:6px 10px;font-size:10px" href="/customer/orders/${o.id}/pay" onclick="event.stopPropagation()"><span>Resume payment</span></a>` : ''}</div>
    </article>`).join('') || '<div class="card"> <p class="muted">Nothing here yet.</p></div>';
  return layout({
  ...
  (function(){
    ...
    function poll(){
      fetch('/api/orders/'+encodeURIComponent('${id}')+'/status').then(function(r){ return r.ok?r.json():null; }).then(function(d){
        ...
        else if(d.status==='READY_FOR_PICKUP') say('Ready for pickup — at the kiosk counter ✓');
      }).catch(function(){});
    }
...
export function orderDetail(user, order, rider, waUrl, fresh) {
  ...
  return layout({
    ...
    ${order.status === 'DELIVERED' ? `<div class="card promo rv" style="margin-bottom:14px"><p class="eyebrow">Liked PrintKarr?</p><p style="font-weight:700;margin:6px 0">Your next print could be free — refer a friend, earn <b>₹20 cash</b>.</p><a class="btn sun" href="/customer/referrals"><span>Get my code →</span></a></div>` : ''}
    ${order.status === 'READY_FOR_PICKUP' ? `<div class="card sun rv" style="margin-bottom:14px"><p class="eyebrow">Ready at the kiosk</p><p style="font-weight:700;margin:6px 0">Your prints are at the counter. Scan the QR there with your phone to mark them collected.</p></div>` : ''}
    <div class="grid c2">
      <div class="card rv">
        ...
        <div class="sumrow"><span>Collect</span><span>${order.deliveryFee === 0 ? '<span class="freebadge">FREE at kiosk</span>' : rs(order.deliveryFee)}</span></div>
