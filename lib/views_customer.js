...
export function customerDashboard(user, { current, pricing, notes, lastDoc, packsHtml }) {
  ...
  return layout({
    ...
    <div class="chips rv">
      ...
      <a class="chip" href="/customer/referrals">Refer &amp; earn print credit</a>
...
export function scanPage(user, order) {
  return layout({
    ...
    body: `
    <p class="eyebrow rv"><a href="/customer/orders/${order.id}" style="text-decoration:none">← Order #${esc(order.id)}</a></p>
    <h1 class="display rv" style="font-size:clamp(2rem,6vw,3.2rem)">Scan the <em>kiosk code.</em></h1>
