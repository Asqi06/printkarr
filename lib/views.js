...
<body>
<div id="offlineBanner" style="display:none;position:fixed;top:0;left:0;right:0;z-index:9999;background:#b91c1c;color:#fff;font-family:var(--font-m);font-size:12px;text-align:center;padding:8px;letter-spacing:.08em">⚠ Offline — orders will sync when back online</div>
...
<main class="login-wrap">
  ...
  <p class="muted mono rv" style="font-size:11px;margin-top:18px">Shop staff? <a class="rowlink" href="/admin/login">Admin login</a></p>
</main>
<div class="toast" id="toast"></div>
<script src="/shell.js" defer></script>
<script>(function(){const b=document.getElementById('offlineBanner');const u=()=>{if(b) b.style.display=navigator.onLine?'none':'block'};window.addEventListener('online',u);window.addEventListener('offline',u);u();})();</script>
<script>
...
<main class="login-wrap" style="max-width:560px">
  ...
  ${error ? `<p class="login-err rv" role="alert">${esc(error)}</p>` : ''}
  ${googleOn ? `<a class="btn solid big rv" style="width:100%;margin-bottom:14px" href="/auth/google"><span>Continue with Google →</span></a>` : ''}
