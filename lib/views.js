...
export const ASSET_V = '20260924-vapi-urgency-pricing';
...
export function pkPublicHeader(active = '') {
  const links = [['/how-it-works', 'How it works'], ['/about', 'Our story'], ['/franchise', 'Host a kiosk'], ['/contact', 'Contact']];
  return `<header class="site-header"><div class="site-header-inner">
    ...
    <div class="header-actions"><a class="account-link" href="/login">My account</a><a class="btn loud" href="/order">Print now <span aria-hidden="true">↗</span></a></div>
    <details class="mobile-menu"><summary aria-label="Open navigation">Menu <span aria-hidden="true">＋</span></summary><nav aria-label="Mobile navigation">${links.map(([href, label]) => `<a href="${href}" ${href === active ? 'aria-current="page"' : ''}>${label}</a>`).join('')}<a href="/login">My account</a><a href="/blogs">Journal</a></nav></details>
  </div></header>`;
}
...
export function pkPublicFooter() {
  ...
  <div><a class="pk-logo" href="/"><span class="w">Print</span><span class="b">Karr</span></a>
    <p class="muted" style="margin-top:16px;max-width:24rem;font-size:14px;line-height:1.6">Your next great idea, on paper. Self-service printing for everyday deadlines, big ideas, and everything in between.</p>
