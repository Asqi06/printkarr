...
export const ASSET_V = '20260926-adminfix';
...
export function pkPublicHeader(active = '') {
  const links = [['/how-it-works', 'How it works'], ['/#pricing', 'Pricing'], ['/#locations', 'Locations'], ['/customer/orders', 'Track Order']];
  return `<header class="site-header"><div class="site-header-inner">
    ...
    <details class="mobile-menu"><summary aria-label="Open navigation">Menu <span aria-hidden="true">＋</span></summary><nav aria-label="Mobile navigation">${links.map(([href, label]) => `<a href="${href}" ${href === active ? 'aria-current="page"' : ''}>${label}</a>`).join('')}<a href="/login">My account</a><a href="/about">Our story</a><a href="/franchise">For businesses</a><a href="/blogs">Journal</a></nav></details>
  </div></header>`;
}
