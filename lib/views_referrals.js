...
export function referralsPage(user, { cfg, code, stats, credit, cash, payouts, shareText }) {
  ...
  return layout({
    ...
    body: `
    <p class="eyebrow rv">Refer &amp; earn · print credit</p>
    <h1 class="display rv" style="font-size:clamp(2rem,6vw,3.6rem)">Their next print<br><em>pays for yours.</em></h1>
    <div class="card promo rv" style="margin-top:16px">
      <p class="eyebrow">How it works</p>
      <div class="sumrow"><span>Friend gets up to ${rs(cfg.friendOff)} off their first eligible purchase of ${rs(cfg.friendMinOrder)}+</span></div>
      <div class="sumrow"><span>You get <b>${rs(cfg.referrerCredit)} print credit</b> after their paid print is collected, or their pack or top-up is paid</span></div>
      <div class="sumrow"><span>Use credit on a future print · rewards capped at ${rs(cfg.monthlyCap)}/month</span></div>
    </div>
    ...
    <div class="grid c2" style="margin-top:14px">
      <div class="card rv"><p class="eyebrow">Friends</p>
        ...
        <div class="sumrow"><span>Qualified</span><span><b>${stats.qualified}</b></span></div>
        <div class="sumrow"><span>Print credit earned</span><span><b>${rs(stats.earned)}</b></span></div>
      </div>
      <div class="card rv"><p class="eyebrow">Milestone bonuses</p>${ms}<p class="muted">Print wallet balance: ${rs(credit.balance)}</p></div>
    </div>
    ${cash.balance > 0 || payouts.length ? `<div class="card rv" style="margin-top:14px">
      <p class="eyebrow">Previous cash earnings</p>
      <h2 class="display" style="font-size:clamp(2rem,6vw,3rem)">${rs(cash.balance)}</h2>
      ${cash.balance >= cfg.minWithdrawal ? `<form method="POST" action="/customer/referrals/withdraw" style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap;align-items:end">
...
export function adminReferralsPage(user, { cfg, referrals, users, payouts }) {
  ...
  const rtrs = referrals.map((r) =>
    `<tr><td class="mono">${esc(r.code)}</td><td>${esc(name(r.referrerId))}</td><td>${esc(name(r.refereeId))}</td>
     <td>${r.sourceKind === 'pack' ? 'pack ' : r.sourceKind === 'topup' ? 'top-up ' : ''}<span class="mono">${esc(r.orderId || r.sourceId || '')}</span></td><td>${esc(r.status)}</td><td><b>−${rs(r.friendDiscount)}</b></td></tr>`
  ...
  return layout({
    ...
    body: `
    <p class="eyebrow rv">Referrals · give up to ${rs(cfg.friendOff)}, get ${rs(cfg.referrerCredit)} print credit</p>
    ...
    <div class="tblwrap rv"><table class="tbl"><thead><tr><th>Code</th><th>Referrer</th><th>Friend</th><th>Source</th><th>Status</th><th>Off</th></tr></thead><tbody>${rtrs}</tbody></table></div>`
  });
}
