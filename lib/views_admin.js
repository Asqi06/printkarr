...
export function adminDashboard(u, d) {
  return layout({
    ...
    body: `
    <p class="eyebrow rv">Printkarr control center</p>
    ...
    <div class="grid c4" style="margin:18px 0">
      ...
      <div class="card rv"><div class="stat"><div class="v">${d.ofd}</div><div class="k">Out for delivery</div></div></div>
    </div>
    <div class="grid c3">
      ...
      <div class="card rv"><div class="stat"><div class="v">${d.delivered}</div><div class="k">Delivered today</div></div></div>
    </div>
...
const FILTERS = ['all', 'new', 'printing', 'ready', 'assigned', 'delivery', 'completed', 'cancelled'];
...
export function orderQueue(u, { filter, q, rows }) {
  ...
  const trs = rows.map((o) => `
    ...
    <td>${badge(o.status)}</td><td>${esc(o.rname || '—')}</td><td><b>${rs(o.total)}</b></td></tr>`).join('')
  ...
  return layout({
    ...
    <div class="tblwrap rv"><table class="tbl"><thead><tr><th>Order</th><th>Customer</th><th>Pages</th><th>Type</th><th>Status</th><th>Rider</th><th>Total</th></tr></thead><tbody>${trs}</tbody></table></div>`
  });
}

export function adminOrderDetail(u, o, c, addr, riders, nexts, rname, waUrl, agentSeen) {
  ...
  const actions = nexts.map((to) => {
    if (to === 'RIDER_ASSIGNED') {
      const opts = riders.map((r) =>
        `<option value="${r.id}">${esc(r.name)} · ${r.kmAway} km ${r.online ? '●' : '○'}</option>`).join('');
      return `<form method="POST" action="/admin/orders/${o.id}/assign" style="display:flex;gap:8px;flex-wrap:wrap;align-items:end">
        <div class="field" style="margin:0"><label>Rider</label><select name="riderId">${opts}</select></div>
        <button class="btn loud" type="submit"><span>Assign rider →</span></button></form>`;
    }
    const label = { PRINT_QUEUE: 'Send to print queue', PRINTING: 'Start printing', PRINTED: 'Mark printed', READY_FOR_PICKUP: 'Mark ready', PICKED_UP: 'Mark picked up', OUT_FOR_DELIVERY: 'Dispatch', DELIVERED: 'Mark delivered', CANCELLED: 'Cancel order', REFUNDED: 'Refund' }[to] || to;
  ...
  return layout({
    ...
    <div class="grid c2">
      <div class="card rv"><p class="eyebrow">Print job</p>
        ...
        ${o.riderId ? `<div class="sumrow"><span>Rider</span><span>${esc(rname || o.riderId)}</span></div>` : ''}
        <div class="sumrow"><span>Deliver</span><span>${esc(addr.address || o.slot)}, ${esc(addr.area || '')}</span></div>
...
export function ridersPage(u, riders) {
  const rows = riders.map((r) => `
    <tr><td><b>${esc(r.name)}</b><br><span class="muted mono" style="font-size:11px">${esc(r.phone || '')}</span></td>
    <td>${r.online ? '● Online' : '○ Offline'}</td><td>${r.active}</td><td><b>${rs(r.week)}</b></td>
    <td><form method="POST" action="/admin/riders/toggle" style="display:inline"><input type="hidden" name="id" value="${r.id}">
    <button class="rowlink" type="submit">${r.online ? 'deactivate' : 'activate'}</button></form></td></tr>`).join('');
  return layout({
    title: 'Riders', user: u, ...A('/admin/riders'),
    body: `
    <p class="eyebrow rv">Riders</p>
    <h1 class="display rv" style="font-size:clamp(2rem,6vw,3.6rem)">The <em>fleet.</em></h1>
    <div class="tblwrap rv" style="margin-top:16px"><table class="tbl"><thead><tr><th>Rider</th><th>Status</th><th>Active</th><th>Week ₹</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>
    <div class="card rv" style="margin-top:14px"><p class="eyebrow">Add rider</p>
      <form method="POST" action="/admin/riders/add" style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap">
        <input name="name" required placeholder="Name" style="flex:1;min-width:140px;border:2px solid var(--ink);border-radius:10px;padding:11px 14px;font:inherit;background:var(--paper)">
        <input name="email" required type="email" placeholder="email@…" style="flex:1;min-width:180px;border:2px solid var(--ink);border-radius:10px;padding:11px 14px;font:inherit;background:var(--paper)">
        <input name="phone" placeholder="Phone" style="flex:1;min-width:140px;border:2px solid var(--ink);border-radius:10px;padding:11px 14px;font:inherit;background:var(--paper)">
        <input name="password" type="password" required minlength="4" placeholder="Login password" style="flex:1;min-width:140px;border:2px solid var(--ink);border-radius:10px;padding:11px 14px;font:inherit;background:var(--paper)">
        <button class="btn sun" type="submit"><span>+ Add</span></button></form></div>`
  });
}
...
export function customerDetailAdmin(u, c, orders, wallet, addresses) {
  ...
  return layout({
    ...
    <form class="card rv" style="margin-top:18px" method="POST" action="/admin/pricing">
      ...
      <div class="grid c2">
        ...
        <div class="field"><label>Campus delivery ₹</label><input name="dz_campus" type="number" min="0" value="${p.delivery.campus}"></div>
      </div>
      <div class="grid c2">
        <div class="field"><label>Vapi delivery ₹</label><input name="dz_vapi" type="number" min="0" value="${p.delivery.vapi}"></div>
        <div class="field"><label>Bhilad delivery ₹</label><input name="dz_bhilad" type="number" min="0" value="${p.delivery.bhilad}"></div>
      </div>
      <div class="grid c2">
        <div class="field"><label>Classroom pickup ₹</label><input name="dz_pickup" type="number" min="0" value="${p.delivery.pickup ?? 0}"></div>
        <div class="field"><label>Free delivery above ₹</label><input name="freeAbove" type="number" min="0" value="${p.freeAbove}"></div>
      </div>
  ...
  return layout({
    ...
    <div class="card rv" style="margin-top:14px"><p class="eyebrow">Create offer</p>
      <form method="POST" action="/admin/coupons" style="margin-top:10px">
        <div class="grid c2"><div class="field"><label>Code</label><input name="code" required placeholder="DIWALI20"></div>
        <div class="field"><label>Type</label><select name="type"><option value="percent">Percentage</option><option value="fixed">Fixed ₹</option></select></div></div>
        <div class="grid c3"><div class="field"><label>Discount</label><input name="value" type="number" min="1" required placeholder="20"></div>
        <div class="field"><label>Min order ₹</label><input name="minOrder" type="number" min="0" value="0"></div>
        <div class="field"><label>Expiry</label><input name="expiry" type="date" required></div></div>
...
export function analyticsPage(u, a) {
  ...
  return layout({
    ...
    body: `
    <p class="eyebrow rv">Analytics</p>
    ...
    <div class="grid c2">
      ...
      <div class="card rv"><p class="eyebrow">Customers &amp; delivery</p>${bar('Repeat customers', a.repeat, a.customers)}${bar('Avg delivery (hrs)', a.avgHrs, Math.max(a.avgHrs, 1))}${bar('Orders / rider', a.perRider, Math.max(a.perRider, 1))}</div>
    </div>`
  });
}
  ...
  return layout({
    ...
    <form class="card rv" style="margin-top:18px" method="POST" action="/admin/settings">
      ...
      <div class="grid c2" style="margin-top:8px">
        <div class="field"><label>Name</label><input name="name" value="${esc(s.business.name)}"></div>
        <div class="field"><label>Phone</label><input name="phone" value="${esc(s.business.phone)}"></div>
      </div>
      <div class="grid c2"><div class="field"><label>Email</label><input name="email" value="${esc(s.business.email)}"></div>
      <div class="field"><label>Address</label><input name="address" value="${esc(s.business.address)}"></div></div>
      <div class="field" style="max-width:320px"><label>Hours</label><input name="hours" value="${esc(s.hours)}"></div>
      <p class="eyebrow">Order limits</p>
      <div class="grid c3" style="margin-top:8px">
        <div class="field"><label>Max file MB</label><input name="maxFileMb" type="number" min="1" value="${s.order.maxFileMb}"></div>
        <div class="field"><label>Max pages</label><input name="maxPages" type="number" min="1" value="${s.order.maxPages}"></div>
        <div class="field"><label>Min total ₹</label><input name="minTotal" type="number" min="0" value="${s.order.minTotal}"></div>
      </div>
